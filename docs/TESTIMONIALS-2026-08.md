# TESTIMONIALS PIPELINE · from the recommendation letters · 2026-08-02

Source: DJ's Google Doc (letters recovered from Gmail/Notes/Dropbox, compiled
...
Jennie Myers (Against, jmyers@goagainst.com): no letter exists anywhere
searched; DJ may nudge. Would be the only Against/agency voice = the
audience-2 gap in this set.

Framing notes for implementation:
- Tommy's cut IS a directed photographer describing DJ's direction -
  attribute exactly that way ("Photographer, directed by DJ" or per DJ).
- The strategy: employees/collaborators -> /about (audience 1, what he's
  like to work for) · client (Sutton) -> the project page as a receipt ·
  shortest two -> home. Letters never mount in full; full letters =
  send-on-request material for hiring managers.
