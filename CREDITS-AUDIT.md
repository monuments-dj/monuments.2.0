# CREDITS AUDIT · generated 2026-07-11

Every rendered role/credit claim, per project. Mark corrections on the CORRECT line
(or just message them); each fix gets propagated to every surface listed.
Disc tag vocabulary: commercial (directing) · cd (creative direction) · photo ·
experiential · campaign (development) · oncam.

## ac-boise
- case page Role lines: Director · the launch film
- case page My part: Directed the launch film
- /work row: "\ Kit reveal film · with Against" · tags [experiential,campaign,cd] · hover "MC-2024-05 · KIT REVEAL"
- home card: role "Director · the launch film" · stats "Directed the launch film|Social + in-person launch|with Against"
- ⚠ MISMATCH: /work tags say creative direction, the page itself never does (the Know Vape pattern)
- CORRECT (leave blank if right): 

## adorama-music
- case page Role lines: Director + Creative Direction
- case page My part: Director + Creative Direction
- /work row: "\ Content series #CreateNoMatterWhat" · tags [cd,commercial,campaign] · hover "MC-2022-07 · #CREATENOMATTERWHAT"
- home card: role "Director + Creative Direction" · stats "Shorty Awards finalist|Launched Adorama Music|Inside the #CreateNoMatterWhat campaign"
- CORRECT (leave blank if right): 

## art-of-visuals
- case page Role lines: Creative Director
- /work row: "\ Brand campaign 4x" · tags [cd,campaign,oncam] · hover "MC-2024-01 · GREW THE AGENCY 4X"
- home card: role "Previous role · Creative Director" · stats "4x agency revenue|Roster · Sony · Intel · MSI · Adorama +|Daily + weekly content engine"
- CORRECT (leave blank if right): 

## blue-cross
- case page Role lines: Creative Direction
- /work row: "\ Documentary campaign 1 week · 23 frames" · tags [cd,photo,campaign,commercial] · hover "MC-2023-04 · REAL PEOPLE, REAL STAKES"
- home card: role "Creative Direction" · stats "Real people, real Idaho|Documentary campaign|with OMEK"
- CORRECT (leave blank if right): 

## buck-the-quo
- case page Role lines: Shooter + editor
- case page My part: Shooter + editor · socials + field experiential
- call sheet: Client: Albertson Family Foundation · Agency: Drake Cooper · Shooter + editor: DJ Ramirez · Socials + field experiential: DJ Ramirez
- /work row: "\ Film series · Rockys" · tags [commercial,cd,oncam] · hover "MC-2021-03 · BEST OF SHOW AT THE ROCKYS"
- home card: role "Shooter + field experiential" · stats "18,000+ conversations|Rockys Best of Show|Statewide activation"
- ⚠ MISMATCH: /work tags say creative direction, the page itself never does (the Know Vape pattern)
- CORRECT (leave blank if right): 

## clothing-merch
- case page Role lines: Creative Director
- case page My part: Creative Director · concept to production
- /work row: "\ Apparel + print" · tags [cd,photo] · hover "MC-2020-08 · APPAREL"
- home card: role "Creative Direction · Design" · stats "Design · print · market|Lines for Creative Space · NYC · DW|Directed the shoot"
- CORRECT (leave blank if right): 

## cwi-lets-get-started
- case page Role lines: Director · Let's Get Started films
- case page My part: Director · the Let's Get Started films
- /work row: "\ Enrollment campaign 3 yrs on air" · tags [campaign,commercial,cd] · hover "MC-2023-05 · 3 YRS ON AIR"
- ⚠ MISMATCH: /work tags say creative direction, the page itself never does (the Know Vape pattern)
- CORRECT (leave blank if right): 

## donut-zumiez
- case page Role lines: Editor · animation + video edit
- case page My part: Editor · animation + video edit
- /work row: "\ Retail collab" · tags [campaign,experiential,cd] · hover "MC-2023-08 · DONUT X ZUMIEZ"
- home card: role "Editor" · stats "Edited the collab video|Animation + video edit|Retail + merch"
- ⚠ MISMATCH: /work tags say creative direction, the page itself never does (the Know Vape pattern)
- CORRECT (leave blank if right): 

## dw-drums
- case page Role lines: Creative direction + design
- case page My part: Creative direction + design
- /work row: "\ Product launch · Design" · tags [commercial,photo] · hover "MC-2022-04 · DW DRUMS"
- home card: role "Creative direction + design" · stats "Design-thinking workshop|How to launch a new product|Drummer-tested"
- ⚠ MISMATCH: page claims creative direction but the /work tags do not
- CORRECT (leave blank if right): 

## flashpoint
- case page Role lines: Creative Director
- case page My part: Creative Director · site + socials
- /work row: "\ Brand build" · tags [cd,campaign] · hover "MC-2020-02 · FLASHPOINT"
- CORRECT (leave blank if right): 

## know-vape
- case page Role lines: Director
- case page My part: Director · all six Myth vs Fact spots
- call sheet: Client: Idaho Public Television · Production: Frame by Frame · Director + Photography: DJ Ramirez · Brand design + motion GFX: Isabel Sarhad
- /work row: "\ PSA campaign" · tags [campaign,commercial] · hover "MC-2021-06 · PUBLIC HEALTH"
- CORRECT (leave blank if right): 

## on-camera
- case page Role lines: Host + Presenter
- /work row: "\ Hosting + presenting · 12 tapes" · tags [oncam] · hover "MC-2026-07 · 12 TAPES, CUED TO MY ENTRANCE"
- CORRECT (leave blank if right): 

## sony-flow-state
- case page Role lines: Director + Creative Director
- case page My part: Director + Creative Director
- call sheet: Client: Sony Audio · Agency: Art of Visuals · Director / Creative Director: DJ Ramirez · Co-director: Krizia Vega · Production: BlackStar ENT · DP: Joachim Zunke · Color: Arianna Shining Star · Stills: Claire LeJune · Deliverables: Videography · Photography · Amplification
- /work row: "\ Campaign film" · tags [commercial,cd] · hover "MC-2023-07 · SONY AUDIO"
- CORRECT (leave blank if right): 

## sony-this-moment
- case page Role lines: Creative Direction
- /work row: "\ Campaign film + stills" · tags [cd,photo,campaign] · hover "MC-2023-06 · SONY AUDIO"
- home card: role "Creative Direction" · stats "Sony Audio campaign|Film + stills|with Art of Visuals"
- CORRECT (leave blank if right): 

## sony-xm5
- case page Role lines: Creative Director
- case page My part: Creative Director · casting + scripts
- /work row: "\ Influencer campaign 1.3M" · tags [campaign,cd] · hover "MC-2022-05 · CREATIVITY STARTS WITH INSPIRATION"
- CORRECT (leave blank if right): 

## sony-xperia-summer
- case page Role lines: Director / Creative Director
- case page My part: Director / Creative Director
- call sheet: Client: Sony Xperia · Agency: Art of Visuals · Director / Creative Director: DJ Ramirez · DP: Casey Levins · Editor: Komin Kovatonic · Stills: DJ Ramirez · Deliverables: Videography · Photography · Amplification
- /work row: "\ Product campaign" · tags [campaign,photo,commercial] · hover "MC-2022-02 · SONY XPERIA"
- ⚠ MISMATCH: page claims creative direction but the /work tags do not
- CORRECT (leave blank if right): 

## turnstile
- case page Role lines: Creative Director
- case page My part: Creative Director
- /work row: "\ Brand + product launch" · tags [cd,campaign,photo] · hover "MC-2023-02 · TURNSTILE AUDIO"
- home card: role "Creative Direction" · stats "Full brand system|Product launch|Naming to packaging"
- CORRECT (leave blank if right): 

## waffle-me-up
- case page Role lines: Creative Director
- case page My part: Creative Director · brand + photo + web
- /work row: "\ Brand + spatial" · tags [cd,experiential,photo] · hover "MC-2019-05 · WAFFLE ME UP"
- CORRECT (leave blank if right): 
